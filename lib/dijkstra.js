...
import { floor1Nodes, floor1Edges } from "./floor1Data";
 ...
 */
export function buildGraph(floor) {
  const floorNodes = floor === "3"
    ? nodes
    : floor === "2"
      ? floor2Nodes
      : floor === "1"
        ? floor1Nodes
        : [...nodes, ...floor2Nodes, ...floor1Nodes];
  const floorEdges = floor === "3"
    ? edges
    : floor === "2"
      ? floor2Edges
      : floor === "1"
        ? floor1Edges
        : [...edges, ...floor2Edges, ...floor1Edges];
  const allNodes = floorNodes;
  ...
  for (const connection of floor ? [] : stairConnections) {
 ...
 */
export function findShortestPath(startNodeId, endNodeId, floor) {
  ...
  const { adjacency } = buildGraph(floor);
